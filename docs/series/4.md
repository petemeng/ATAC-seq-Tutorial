...
经过前一篇的质控和接头去除，你手上已经有了四个干干净净的 FASTQ 文件。但 FASTQ 里只有一条条"悬浮"的序列——你不知道它们来自基因组的哪个位置，也无法判断哪里的染色质是开放的。**比对（alignment），就是把这些 reads 放回它们在基因组上的位置。** 这一步是后续所有分析——从 peak calling 到 motif 分析——的地基。地基没打好，上层建筑全白搭。
...
# ============================================================
mkdir -p ref/bt2_index && cd ref/bt2_index
...
# ============================================================
ls -lh ref/bt2_index/hg38*.bt2
```

```
📊 输出：
-rw-r--r-- 1 user user 886M Apr 10 09:12 ref/bt2_index/hg38.1.bt2
-rw-r--r-- 1 user user 660M Apr 10 09:12 ref/bt2_index/hg38.2.bt2
-rw-r--r-- 1 user user  52K Apr 10 09:12 ref/bt2_index/hg38.3.bt2
-rw-r--r-- 1 user user 660M Apr 10 09:12 ref/bt2_index/hg38.4.bt2
-rw-r--r-- 1 user user 886M Apr 10 09:34 ref/bt2_index/hg38.rev.1.bt2
-rw-r--r-- 1 user user 660M Apr 10 09:34 ref/bt2_index/hg38.rev.2.bt2
```
...
bowtie2 --very-sensitive -X 2000 --no-mixed --no-discordant \
    --threads 8 -x ref/bt2_index/hg38 \
    -1 clean/GM12878_rep1_R1_clean.fastq.gz \
    -2 clean/GM12878_rep1_R2_clean.fastq.gz \
    2> logs/GM12878_rep1_bowtie2.log | \
    samtools sort -@ 8 -O BAM -o aligned/GM12878_rep1.bam

samtools index aligned/GM12878_rep1.bam
```
...
```
📊 输出：
51284630 reads; of these:
  51284630 (100.00%) were paired; of these:
    1782105 (3.47%) aligned concordantly 0 times
    47168224 (91.97%) aligned concordantly exactly 1 time
    2334301 (4.55%) aligned concordantly >1 times
96.53% overall alignment rate
```

几个关键指标：

1. **总 reads 数（51,284,630）**：这是经过质控后进入比对的 reads 数，和上一篇 fastp 输出的 clean reads 数应该一致。

2. **Concordant 0 times（3.47%）**：这些 reads 找不到合适的配对比对位置。低于 5% 是正常的，如果这个比例超过 10%，需要检查参考基因组版本是否正确、是否有严重的物种污染。

3. **Concordant exactly 1 time（91.97%）**：这是我们最想要的——reads 唯一地比对到基因组上的一个位置。**这个比例越高越好，ATAC-seq 中通常在 85-95% 之间。**

4. **Concordant >1 times（4.55%）**：这些 reads 可以比对到多个位置（多重比对），通常来自重复序列区域。后续过滤步骤会用 MAPQ 阈值处理这些 reads。

5. **Overall alignment rate（96.53%）**：总体比对率。ATAC-seq 中 **95-99%** 是正常范围。

💡 **什么情况说明比对出了问题？**
> - 总体比对率 <90%：检查参考基因组版本、是否有 adapter 残留、是否有外源污染
> - Concordant exactly 1 time <80%：基因组重复区域占比过高，或文库复杂度低
...
# ============================================================
samtools flagstat aligned/GM12878_rep1.bam
```

```
📊 输出：
102569260 + 0 in total (QC-passed reads + QC-failed reads)
99005050 + 0 primary
0 + 0 secondary
3564210 + 0 supplementary
0 + 0 duplicates
99434958 + 0 primary mapped (100.00% : N/A)
99005050 + 0 paired in sequencing
49502525 + 0 read1
49502525 + 0 read2
99005050 + 0 properly paired (100.00% : N/A)
99005050 + 0 with itself and mate mapped
0 + 0 singletons (0.00% : N/A)
816244 + 0 with mate mapped to a different chr
412038 + 0 with mate mapped to a different chr (mapQ>=5)
```
...
| 字段 | 数值 | 含义 |
|------|------|------|
| in total | 102,569,260 | BAM 中的总记录数（包括 supplementary） |
| primary | 99,005,050 | 主要比对记录（= paired reads × 2，因为 R1 和 R2 各算一条） |
| secondary | 0 | 次要比对（Bowtie2 默认不输出） |
| supplementary | 3,564,210 | 补充比对（嵌合比对的额外片段） |
| properly paired | 100.00% | 正确配对率。因为我们用了 `--no-mixed --no-discordant`，所以未配对的 reads 直接被排除了 |
| singletons | 0.00% | 无单端比对，因为 `--no-mixed` 参数的效果 |
| mate mapped to diff chr | 816,244 | 配对比到了不同染色体，可能是结构变异或比对噪音 |
...
for sample in "${samples[@]}"; do
    ...
    bowtie2 --very-sensitive -X 2000 --no-mixed --no-discordant \
        --threads 8 -x ref/bt2_index/hg38 \
        -1 clean/${sample}_R1_clean.fastq.gz \
        -2 clean/${sample}_R2_clean.fastq.gz \
        2> logs/${sample}_bowtie2.log | \
        samtools sort -@ 8 -O BAM -o aligned/${sample}.bam
...
>     --threads 8 -x ref/bt2_index/hg38 \
>     -1 clean/{}_R1_clean.fastq.gz -2 clean/{}_R2_clean.fastq.gz \
>     2> logs/{}_bowtie2.log | samtools sort -@ 4 -O BAM -o aligned/{}.bam && \
>     samtools index aligned/{}.bam' ::: GM12878_rep1 GM12878_rep2 K562_rep1 K562_rep2
> ```
...
# ============================================================
echo -e "样本\t总reads\tConcordant_0\tConcordant_1\tConcordant_>1\t比对率" \
...
```
📊 输出：
样本           总reads     Concordant_0  Concordant_1  Concordant_>1  比对率
GM12878_rep1   51284630    1782105       47168224       2334301        96.53%
GM12878_rep2   49873216    1685430       45927481       2260305        96.62%
K562_rep1      52107844    1276528       48356012       2475304        97.55%
K562_rep2      50568322    1194403       46881730       2492189        97.64%
```

看一下这张表，几个要点：

- **四个样本的比对率都在 96-98%**，非常健康。K562 的比对率略高于 GM12878，这在不同细胞系之间很正常。
- **Concordant exactly 1 time 占绝对多数**（>90%），说明大部分 reads 能唯一定位到基因组上，这对后续分析至关重要。
- **多重比对（>1 times）约 4-5%**，这些 reads 主要来自基因组的重复序列区域，后续用 MAPQ 过滤可以处理掉。

<!-- 图 1 位置：四个样本的比对率柱状图，包含 concordant 0/1/>1 times 的堆叠比例 -->
**图 1：四个样本的比对率汇总。** 所有样本的总体比对率均 >96%，唯一比对占比 >90%，说明比对质量优秀，可以放心进入后续分析。
...
- 比对率 >95%、唯一比对率 >85% 是 ATAC-seq 数据的健康指标
