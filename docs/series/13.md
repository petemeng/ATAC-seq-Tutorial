...
for sample in GM12878_Rep1 GM12878_Rep2 K562_Rep1 K562_Rep2; do
    ...
    samtools view -h filtered/${sample}_shifted.bam | \
        awk 'substr($0,1,1)=="@" || (($9>0?$9:-$9) < 150)' | \
        samtools view -bS - > nfr/${sample}_NFR.bam
    ...
    samtools view -h filtered/${sample}_shifted.bam | \
        ...
        samtools view -bS - > nuc/${sample}_mono.bam
    ...
    total_count=$(samtools view -c filtered/${sample}_shifted.bam)
...
>>> 正在处理 GM12878_Rep1 ...
  总 reads: 48,235,612
  NFR (<150bp): 27,845,230 (57.7%)
  Mono-nuc (150-300bp): 13,218,456 (27.4%)

>>> 正在处理 K562_Rep1 ...
  总 reads: 52,108,934
  NFR (<150bp): 30,125,677 (57.8%)
  Mono-nuc (150-300bp): 14,892,301 (28.6%)
```

这里有一个关键的质控指标：**NFR reads 的比例应该在 50–70% 之间**。如果 NFR 比例太低，可能意味着 Tn5 浓度不足或细胞裂解不彻底；如果太高（>80%），可能意味着过度消化。我们的数据 ~58% 落在 NFR 区间，非常理想。
...
**图 1：TSS 处 NFR 与 mono-nucleosomal 信号分布。** 上方 Profile 图显示了所有蛋白编码基因 TSS ±2 kb 范围内的平均信号。**NFR 信号（蓝色）在 TSS 正上方呈现尖锐单峰**，表明转录起始位点处核小体被清除、染色质高度可及；**Mono-nuc 信号（红色）在 TSS 两侧各出现一个峰**，对应 +1 和 -1 定位核小体，它们在 TSS 两侧形成"路障"，为转录因子划出一块可进入的窗口。下方 Heatmap 按 NFR 信号强度排序，可以看到表达量高的基因（上部）NFR 信号最强、核小体排列最规律，而低表达基因（下部）信号微弱、核小体排列更杂乱。
...
for sample in GM12878_Rep1 GM12878_Rep2 K562_Rep1 K562_Rep2; do
    ...
    nucleoatac run \
        ...
        --bam filtered/${sample}_shifted.bam \
        --fasta ref/hg38.fa \
...
gm_nfr <- read_tsv("nucleoatac/GM12878_Rep1.nfrpos.bed.gz",
                    col_names = c("chr", "start", "end", "id",
                                  "score", "nfr_width", "log_likelihood"),
